...
class Order(Base):
    ...
    def __repr__(self):
        return "Order(%r, %r)" % (self.order_date, self.customer_name)
...
if __name__ == "__main__":
    ...
    #order = select(Order).filter_by(customer_name="john smith")
    q = select(Order).filter_by(customer_name="john smith")
    order = session.execute(q).one()
